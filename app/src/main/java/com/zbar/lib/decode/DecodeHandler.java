...
final class DecodeHandler extends Handler {
	...
	private void decode(byte[] data, int width, int height) {
		...
		String result = manager.decode(rotatedData, width, height, true, fragment.getX(), fragment.getY(), fragment.getCropWidth(),
				fragment.getCropHeight());

		if (result != null) {
			if (fragment.isNeedCapture()) {
				// 生成bitmap
				PlanarYUVLuminanceSource source = new PlanarYUVLuminanceSource(rotatedData, width, height, fragment.getX(), fragment.getY(),
						fragment.getCropWidth(), fragment.getCropHeight(), false);
			...
			if (null != fragment.getHandler()) {
				Message msg = new Message();
				msg.obj = result;
				msg.what = R.id.decode_succeeded;
				fragment.getHandler().sendMessage(msg);
			}
		} else {
			if (null != fragment.getHandler()) {
				fragment.getHandler().sendEmptyMessage(R.id.decode_failed);
			}
		}
	}
